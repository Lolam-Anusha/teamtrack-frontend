...
const Table = () => {
    ...
    const fetchLeaves = async () => {
        try {
            const response = await axios.get("https://teamtrack-backend.vercel.app/api/leave", {
                headers: {
    ...
    const filterByInput = (e) => {
        const data = leaves.filter(leave => leave.employeeId.toLowerCase()
        .includes(e.target.value.toLowerCase()))
        setFilteredLeaves(data)
    }
    const filterByButton = (status) => {
        const data = leaves.filter(leave => leave.status.toLowerCase()
